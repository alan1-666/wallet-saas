...
import (
	...
	"wallet-saas-v2/services/chain-gateway/internal/adapters/account/chains/evmbase"
	erc20Base "wallet-saas-v2/services/chain-gateway/internal/adapters/account/chains/evmbase"
...
func NewChainAdaptor(conf *config.Config) (chain.IChainAdaptor, error) {
	ethClient, err := erc20Base.DialEthClient(context.Background(), conf.WalletNode.Scroll.RpcUrl)
	if err != nil {
		return nil, err
	}
	ethDataClient, err := erc20Base.NewEthDataClient(conf.WalletNode.Scroll.DataApiUrl, conf.WalletNode.Scroll.DataApiKey, time.Duration(conf.WalletNode.Scroll.TimeOut))
	if err != nil {
		return nil, err
	}
	return &ChainAdaptor{
