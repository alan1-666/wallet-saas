...
func NewChainAdaptor(conf *config.Config) (chain.IChainAdaptor, error) {
	ethClient, err := erc20Base.DialEthClient(context.Background(), conf.WalletNode.Mantle.RpcUrl)
	if err != nil {
		return nil, err
	}
	ethDataClient, err := erc20Base.NewEthDataClient(conf.WalletNode.Mantle.DataApiUrl, conf.WalletNode.Mantle.DataApiKey, time.Duration(conf.WalletNode.Mantle.TimeOut))
	if err != nil {
		return nil, err
	}
	return &ChainAdaptor{
