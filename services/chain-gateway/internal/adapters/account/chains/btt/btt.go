...
func NewChainAdaptor(conf *config.Config) (chain.IChainAdaptor, error) {
	ethClient, err := erc20Base.DialEthClient(context.Background(), conf.WalletNode.Btt.RpcUrl)
	if err != nil {
		return nil, err
	}
	ethDataClient, err := erc20Base.NewEthDataClient(conf.WalletNode.Btt.DataApiUrl, conf.WalletNode.Btt.DataApiKey, time.Duration(conf.WalletNode.Btt.TimeOut))
	if err != nil {
		return nil, err
	}
	return &ChainAdaptor{
