...
// Adapter multiplexes utxo-model plugins by chain.
type Adapter struct {
	mu      sync.RWMutex
...
func (a *Adapter) resolve(chain string) (ChainPlugin, error) {
	...
	a.mu.RUnlock()
	if !ok {
		return nil, fmt.Errorf("utxo plugin not configured chain=%s", normalized)
	}
	return plugin, nil
}
...
func (a *Adapter) BuildUnsignedUTXO(ctx context.Context, chain, network, fee string, vin []ports.TxVin, vout []ports.TxVout) (ports.BuildUnsignedResult, error) {
	...
	return plugin.BuildUnsignedUTXO(ctx, clients.NormalizeChain(chain), strings.ToLower(strings.TrimSpace(network)), fee, vin, vout)
}
...
func (a *Adapter) BuildSignedUTXO(ctx context.Context, chain, network string, txData []byte, signatures [][]byte, publicKeys [][]byte) ([]byte, string, error) {
	...
	return plugin.BuildSignedUTXO(ctx, clients.NormalizeChain(chain), strings.ToLower(strings.TrimSpace(network)), txData, signatures, publicKeys)
}
...
func (a *Adapter) GetUnspentOutputs(ctx context.Context, chain, network, address string) (json.RawMessage, error) {
	...
	return plugin.GetUnspentOutputs(ctx, clients.NormalizeChain(chain), strings.ToLower(strings.TrimSpace(network)), address)
}
