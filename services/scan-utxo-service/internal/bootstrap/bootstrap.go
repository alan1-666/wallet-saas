...
import (
	...
	"wallet-saas-v2/services/scan-utxo-service/internal/config"
	"wallet-saas-v2/services/scan-utxo-service/internal/store"
	"wallet-saas-v2/services/scan-utxo-service/internal/worker"
)
...
func runWithMode(mode string) error {
	...
	log.Printf("service=scan-utxo-service mode=%s wallet-core=%s chain-gateway-grpc=%s project-notify=%s interval=%ds addr-concurrency=%d sweep-min-balance=%s reorg-window=%d reorg-candidate-limit=%d reorg-not-found-threshold=%d",
